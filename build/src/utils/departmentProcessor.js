...
exports.departmentProcessor = exports.default = void 0;
...
class DepartmentProcessor {
  ...
  async start() {
    try {
      ...
      _messageQueue.messageQueue.subscribe(/department-.*-queue/, this.processDepartmentQueueMessage.bind(this));
  ...
  async processDepartmentQueueMessage(message) {
    ...
    try {
      ...
      const doctors = await _Doctor.default.find({
        specialization,
        isOnline: true
      });
      if (doctors.length === 0) {
      ...
      await _priorityQueue.priorityQueue.addAppointmentToQueue(selectedRoom, message);
      console.log(`Bệnh nhân ${patientId} đã được phân công cho bác sĩ ${selectedDoctor._id}`);

      // Cập nhật appointment với doctorId
      const appointment = await _Appointment.default.findOne({
        patientId
      });
      if (appointment) {
  ...
  async processPendingMessages() {
    ...
    const departmentTopics = Array.from(_messageQueue.messageQueue.queues.keys()).filter(topic => topic.startsWith('department-') && topic.endsWith('-queue'));
    for (const topic of departmentTopics) {
      const message = _messageQueue.messageQueue.dequeue(topic);
      if (message) {
  ...
  getStats() {
    return {
      ...
      queueStats: _messageQueue.messageQueue.getStats()
    };
  }
}

// Singleton instance
const departmentProcessor = exports.departmentProcessor = new DepartmentProcessor();
var _default = exports.default = DepartmentProcessor;