...
class PriorityQueue extends EventEmitter {
  ...
  async addAppointmentToQueue(roomNumber, patientData) {
    try {
      if (!this.queues.has(roomNumber)) {
        this.queues.set(roomNumber, { priority: [], normal: [] });
      }
      ...
      this.emit('queueUpdated', { roomNumber, patientData, action: 'added' });
  ...
  async addPrescriptionToQueue(prescriptionData) {
    try {
      ...
      if (!this.queues.has(queueKey)) {
        this.queues.set(queueKey, { priority: [], normal: [] });
      }
  ...
  async addRequestTestToQueue(testName, requestTestData) {
    try {
      ...
      this.emit('testRequestAdded', { testName, requestTestData });
  ...
  async getNextPatientFromQueue(roomNumber) {
    try {
      ...
      if (patientData) {
        ...
        this.emit('patientProcessed', { roomNumber, patientData });
      } else {
        console.log(`No patients in queue for room ${roomNumber}`);
      }
  ...
  async removePatientFromQueue(roomNumber, patientId) {
    try {
      ...
      const priorityIndex = queue.priority.findIndex(patient => 
      ...
      if (priorityIndex !== -1) {
        ...
        this.emit('patientRemoved', { roomNumber, patientId });
        return true;
      }

      // Tìm trong normal queue
      const normalIndex = queue.normal.findIndex(patient => 
        patient.patientId === patientId
      );
