...
class InMemoryQueueRepository {
  ...
  async getAppointmentsFromQueueRepo(queueKey) {
    try {
      ...
      const patients = await priorityQueue.getAllPatientsFromQueue(roomNumber);
  ...
  async removeFromQueueRepo(queueKey, patientDataString) {
    try {
      ...
      return await priorityQueue.removePatientFromQueue(roomNumber, patientData.patientId);
    } catch (err) {
  ...
  async addToQueueRepo(queueKey, patientData) {
    try {
      const roomNumber = queueKey.replace('queue:', '');
      return await priorityQueue.addAppointmentToQueue(roomNumber, patientData);
    } catch (err) {
  ...
  async getNextFromQueueRepo(queueKey) {
    try {
      const roomNumber = queueKey.replace('queue:', '');
      return await priorityQueue.getNextPatientFromQueue(roomNumber);
    } catch (err) {
  ...
  async getQueueStatsRepo(queueKey) {
    try {
      const roomNumber = queueKey.replace('queue:', '');
      return priorityQueue.getQueueStats(roomNumber);
    } catch (err) {
  ...
  async clearQueueRepo(queueKey) {
    try {
      const roomNumber = queueKey.replace('queue:', '');
      const queue = priorityQueue.queues.get(roomNumber);
  ...
  async queueExists(queueKey) {
    try {
      const roomNumber = queueKey.replace('queue:', '');
      return priorityQueue.queues.has(roomNumber);
    } catch (err) {
  ...
  async getQueueSize(queueKey) {
    try {
      const roomNumber = queueKey.replace('queue:', '');
      const stats = priorityQueue.getQueueStats(roomNumber);
      return stats.total;
    } catch (err) {
...
export default InMemoryQueueRepository;
